...
def test_diff_a_wins_on_two_tasks() -> None:
    a = make_summary(["tb-1", "tb-2", "tb-3"], ["tb-4"])
    b = make_summary(["tb-1"], ["tb-2", "tb-3", "tb-4"])
    d = compute_diff(a, b)
...
def test_all_fail_renders_with_zero_delta() -> None:
    a = load_summary(fixture_root("case-all-fail") / "agent-a" / "summary.json")
    b = load_summary(fixture_root("case-all-fail") / "agent-b" / "summary.json")
    d = compute_diff(a, b)
...
def test_identical_results_yield_zero_deltas() -> None:
    a = load_summary(fixture_root("case-identical-results") / "agent-a" / "summary.json")
    b = load_summary(fixture_root("case-identical-results") / "agent-b" / "summary.json")
    d = compute_diff(a, b)
    assert d["delta_pass_rate"] == 0.0
    assert d["delta_passed"] == 0
    assert d["tasks_agent_a_passed_b_failed"] == []
...
def test_missing_agent_b_raises_file_not_found() -> None:
    ...
    path = fixture_root("case-agent-b-missing") / "agent-b" / "summary.json"
