...
def test_main_returns_zero_when_build_event_fails(capsys: pytest.CaptureFixture) -> None:
    payload = json.dumps({"tool_name": "Edit"})
    with patch.object(sys, "stdin") as mock_stdin, patch.object(tc, "_build_event", side_effect=RuntimeError("boom")):
        mock_stdin.read.return_value = payload
        assert tc.main() == 0
    captured = capsys.readouterr()
