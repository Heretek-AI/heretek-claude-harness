...
## Subagent execution
- For 5+ files of creative content, watch the dispatched agent's mtime every ~5 min. Hard-gate: no commit and no disk growth for 5 min → take over and finish the work yourself. Subagents that are "still working" without observable progress are stalls.
...
- `subprocess.run(["git", ...])` in tests without `_clean_env()` (pre-commit sets GIT_DIR → wrong gitdir → tests fail under ci.sh but pass under direct pytest)
