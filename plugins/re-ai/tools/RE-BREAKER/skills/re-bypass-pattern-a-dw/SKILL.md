---
name: re-bypass-pattern-a-dw
version: 0.2.0
status: implemented
family: encrypted-vm-bytecode-interpreter
severity: critical
catalog_entry: encrypted-vm.bytecode-interpreter.pattern-a-dw
playbook: docs/PLAYBOOKS/pattern-a-dw-denuvo.md
pattern_yaml: data/patterns/pattern-a-dw.yml
---

# re-bypass-pattern-a-dw

**v0.2.0 implemented.** Pattern A-DW: encrypted-VM bytecode interpreter + Denuvo ATD (UE5 variant)

## When to use this skill

re-catalog-match returns encrypted-vm.bytecode-interpreter.pattern-a-dw (high confidence). Target has the full encrypted-VM section set + a POGO entry + kernel-active hypervisor posture. 200+ RDTSC + 200+ CPUID + 36 VMCALL sites.

## Tools invoked

- `mcp__re_anti_debug_patch.*` — re-anti-debug-patch
- `mcp__re_vm_decrypt.*` — re-vm-decrypt
- `mcp__re_runtime_dump.*` — re-runtime-dump
- `mcp__re_encrypted_vm_bypass.*` — re-encrypted-vm-bypass
...
1. **Confirm the catalog match.** Run `mcp__re-catalog-match.match_catalog(target, intent="offender")` and verify the match is `encrypted-vm.bytecode-interpreter.pattern-a-dw` with high confidence.
...
- **RE-UNLEASHED doc**: `protection/encrypted-vm-bytecode-interpreter/pattern-a-dw-ue5.md` —
...
- [Catalog entry this skill implements](../../data/catalog.json) — `encrypted-vm.bytecode-interpreter.pattern-a-dw`
- [Pattern YAML](../../data/patterns/pattern-a-dw.yml)
- [Playbook](../../docs/PLAYBOOKS/pattern-a-dw-denuvo.md)
