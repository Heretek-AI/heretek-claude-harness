---
name: re-bypass-pattern-c
version: 0.2.0
status: implemented
family: encrypted-vm-bytecode-interpreter
severity: critical
catalog_entry: encrypted-vm.bytecode-interpreter.pattern-c
playbook: docs/PLAYBOOKS/ioi-glacier-shielding.md
pattern_yaml: data/patterns/pattern-c.yml
---

# re-bypass-pattern-c

**v0.2.0 implemented.** Pattern C: encrypted-VM bytecode interpreter (proprietary-engine target)

## When to use this skill

re-catalog-match returns encrypted-vm.bytecode-interpreter.pattern-c. Section set is the same as Pattern A but the engine is proprietary (not UE5, not Unity). 1966 RDTSC + 200+ CPUID + 200+ VMXON sites.
...
1. **Confirm the catalog match.** Run `mcp__re-catalog-match.match_catalog(target, intent="offender")` and verify the match is `encrypted-vm.bytecode-interpreter.pattern-c` with high confidence.
...
- **RE-UNLEASHED doc**: `protection/encrypted-vm-bytecode-interpreter/pattern-c-proprietary.md` —
...
- [Catalog entry this skill implements](../../data/catalog.json) — `encrypted-vm.bytecode-interpreter.pattern-c`
