---
name: re-bypass-pattern-a-vmt
version: 0.2.0
status: implemented
family: encrypted-vm-bytecode-interpreter
severity: critical
catalog_entry: encrypted-vm.bytecode-interpreter.pattern-a-vmt
playbook: docs/PLAYBOOKS/pattern-a-vmt-blackspace.md
pattern_yaml: data/patterns/pattern-a-vmt.yml
---

# re-bypass-pattern-a-vmt

**v0.2.0 implemented.** Pattern A-VMT: encrypted-VM handler-table dispatch (BlackSpace engine)

## When to use this skill

re-catalog-match returns encrypted-vm.bytecode-interpreter.pattern-a-vmt. Section set includes .xcode + .link + .arch (the dual-regime handler-table dispatch). 2390 RDTSC + 26 VMCALL + 136 VMX-EPT sites.

## Tools invoked

- `mcp__re_vm_decrypt.*` — re-vm-decrypt
- `mcp__re_frida.*` — re-frida
...
1. **Confirm the catalog match.** Run `mcp__re-catalog-match.match_catalog(target, intent="offender")` and verify the match is `encrypted-vm.bytecode-interpreter.pattern-a-vmt` with high confidence.
...
- **RE-UNLEASHED doc**: `protection/encrypted-vm-bytecode-interpreter/pattern-a-vmt.md` —
...
- [Catalog entry this skill implements](../../data/catalog.json) — `encrypted-vm.bytecode-interpreter.pattern-a-vmt`
