...
Mirrors ``android-re-static``'s project_tools so the native server is
self-contained when used without the static server (e.g. from the
``android-re-native-triage`` skill when only the LIEF-backed report
tool is needed). The static and native servers maintain separate
process-local ``ProjectStore``s; opening the same APK in both gives
matching ``project_id``s but does NOT share an in-memory record.
"""
...
def register(mcp: FastMCP) -> None:
    ...
    @mcp.tool(
        name="open_project",
        description=(
            "Open an APK file and register a new project on the native "
            "server's store. Returns a project_id that you must pass to "
            "every subsequent native tool. Re-opening the same APK returns "
            "the existing project. Performs zip-bomb and size checks before "
            "parsing. The default size cap is 500 MB; pass `max_size` to "
            "raise it for a single call."
        ),
    )
    def open_project(
        apk_path: Annotated[
    ...
    @mcp.tool(
        name="list_projects",
        description="List all currently open projects on the native server.",
    )
